...
public class PaymentController {
  ...
  public CommonsResult<Integer> createPayment( @RequestBody Payment payment) {//必须加上RequestBody否则restTemplate传来的参数，接收不到
    log.info("payment8002--> PaymentController -> createPayment接受到的参数是{}",payment);
    Integer integer = paymentService.createPayment(payment);
    log.info("payment8002--> PaymentController -> createPayment返回的结果是{}",integer);
    if(integer > 0){
  ...
  public CommonsResult<List<Payment>> findAllPayment() {
    ...
    log.info("payment8002--> PaymentController -> findAllPayment查询所有的结果集{}",allPayment);
  ...
  public @ResponseBody CommonsResult<Payment> findPaymentById(@PathVariable("id") Long id) {
    log.info("payment8002--> PaymentController -> findPaymentById需要查询的id是{}",id);
    Payment payment = paymentService.findPaymentById(id);
    log.info("payment8002--> PaymentController -> findPaymentById根据id查到的结果{}",payment);
    if (payment != null) {
      return new CommonsResult(200,"8002查询成功",payment);
    } else {
      return new CommonsResult(401,"8002查询失败");
    }
  }
